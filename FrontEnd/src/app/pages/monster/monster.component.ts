...
export class MonsterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const formValuesSubscriptions = this.formGroup.valueChanges.subscribe(formData => {
      this.monster = Object.assign(new Monster(), formData);
    });
    this.subscriptions.add(formValuesSubscriptions);
    
    const routeSubscriptions = this.route.params.pipe(
      switchMap(params => {
        if (params["id"]){
    ...
    this.subscriptions.add(routeSubscriptions);
  }
